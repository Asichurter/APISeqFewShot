...
# drop the redundant tail output
...
class TemporalConvNet(nn.Module):
    def __init__(self,
        ...
        for i in range(num_levels):
            ...
            # here, use '(kernel-1)*dilation' to enforce the i-th output can only
            # associate the i-th input furthest, but result in redundant tail output
...
if __name__ == '__main__':
    # block = TemporalBlock(1,1,3,1,2,2,dropout=0)
    net = TemporalConvNet(1,[2,4,8],kernel_size=3)
    inp = torch.FloatTensor([[[0.5, 1.5, 2, -0.5, 1, 2, 3, 4, 5, 6]]])
