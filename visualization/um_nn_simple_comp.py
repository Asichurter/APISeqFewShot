...
def SIMPLE(X, Y, init_proto, init_proto_label, iter=3, sigma=1):
    def cal_assigned_logits(data, protos, p_labels):
        ...
        # generate penalization term for unmatched point and proto
        N_INF = -100
        class_mask_matrix = np.zeros_like(protos)
        y_expand = np.expand_dims(Y, 1).repeat(proto_len,1).reshape(data_len*proto_len)
        p_label_expand = np.expand_dims(p_labels, 0).repeat(data_len,0).reshape(data_len*proto_len)
        class_mask = (y_expand!=p_label_expand)[:,None].repeat(2,-1)
        np.putmask(class_mask_matrix, class_mask, N_INF)

        # logits = (-(data-protos)**2).sum(-1)/(2*sigma) - norm
        logits = (-((data-protos)**2)+class_mask_matrix).sum(-1)/(2*sigma) - norm
    ...
    for i in range(iter):
        ...
        logits_ = cal_assigned_logits(X,protos,proto_labels).T
